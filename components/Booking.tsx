...
export default function Booking() {
  ...
  return (
    <section id="booking" className="py-28 px-6 relative">
      <div className="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 w-[700px] h-[400px] bg-[#00FF87] opacity-[0.04] blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div
          ...
          className="gradient-border rounded-2xl bg-[#0f1117] overflow-hidden"
        >
          {/* Calendly embed placeholder */}
          <div className="flex flex-col items-center justify-center py-20 px-8 gap-6">
            {/* Calendly UI simulation */}
            <div className="w-16 h-16 rounded-2xl bg-[#00FF87]/10 border border-[#00FF87]/20 flex items-center justify-center text-3xl">
              📅
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 w-full max-w-sm">
              {["Mon 14 Apr", "Tue 15 Apr", "Wed 16 Apr", "Thu 17 Apr", "Fri 18 Apr", "Mon 21 Apr"].map(
                (slot) => (
                  <button
                    key={slot}
                    className="px-3 py-2 rounded-lg border border-[#1e2433] text-xs text-gray-400 hover:border-[#00FF87]/40 hover:text-[#00FF87] transition-all duration-200 text-center"
                  >
                    {slot}
                  </button>
                )
            ...
            <a
              ...
              className="px-8 py-4 rounded-xl bg-[#00FF87] text-black font-bold text-base hover:bg-[#00cc6a] transition-all duration-200 hover:scale-[1.03] active:scale-95 mt-2"
              style={{ boxShadow: "0 0 30px rgba(0,255,135,0.25)" }}
            >
              Book Your Free Call →
            </a>
